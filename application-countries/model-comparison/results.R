...
############################################################
#### Flags for additional experiments (end of scripts) #####
############################################################

## Post-process results for NGP and Kalman
# It requires to run before the respective experiments in
# BSP-mortality/application-countries/model-comparison/
flag_NGP_Kalman <- FALSE

## Results with extra countries 
# It requires to run the experiments in 
# BSP-mortality/application-countries/model-comparison/extra
flag_extra_countries <- FALSE
...
if(flag_NGP_Kalman)
{
  ...
  bind_rows(NGP_res_age,
    ...
    summarise(median_abserr_log = median(mad_log),
              ...
              q75_abserr_log = quantile(mad_log,  probs = 0.75),
              coverage = mean(inside)) %>%
    ungroup() -> results_NGP_Kalman
  
  print(results_NGP_Kalman)
}


if(flag_extra_countries)
{
